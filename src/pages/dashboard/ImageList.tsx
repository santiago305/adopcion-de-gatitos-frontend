...
const ImageList: React.FC = () => {
  ...
  const backendPort = '3000'; // Ajusta este puerto según el valor en main.ts del backend
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {images.map((imagePath, index) => (
          <div key={index} className="p-4 bg-white rounded-lg shadow-md">
            <img
              src={`http://localhost:${backendPort}/${imagePath}`}
              alt={`Imagen ${index + 1}`}
              className="w-full h-64 object-cover rounded-md"
              onError={() => setError(`No se pudo cargar ${imagePath}. Verifica el archivo o el puerto.`)}
            />
