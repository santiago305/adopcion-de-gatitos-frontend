...
export default function MobileMenuButton({ isOpen, toggle }: Props) {
  return (
    <button onClick={toggle} className="relative w-8 h-8 flex flex-col justify-center items-start gap-1 z-50">
      {/* Línea Superior */}
      <motion.span
        animate={{ rotate: isOpen ? 45 : 0, y: isOpen ? 8 : 0, width: isOpen ? '100%' : '50%' }}
      ...
      {/* Línea Inferior */}
      <motion.span
        animate={{ rotate: isOpen ? -45 : 0, y: isOpen ? -8 : 0, width: isOpen ? '100%' : '100%' }}
        transition={{ duration: 0.3 }}
